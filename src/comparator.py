...
class PackagesComparer:
    ...
    @classmethod
    def _parse_version(cls, version_string):
        """
    ...
    @classmethod
    def _compare_elements(cls, element1, element2):
        """
    ...
    def _compare_versions(cls, version1, version2):
        ...
        Returns:
            int: -1, 0, or 1
        """
        epoch1, ver1, rel1 = cls._parse_version(version1)
        epoch2, ver2, rel2 = cls._parse_version(version2)

        if epoch1 != epoch2:
            return epoch1 - epoch2

        for subver1, subver2 in zip(ver1.split("."), ver2.split(".")):
            ver_comp = cls._compare_elements(subver1, subver2)
            if ver_comp != 0:
                return ver_comp

        for subrel1, subrel2 in zip(rel1.split("."), rel2.split(".")):
            rel_comp = cls._compare_elements(subrel1, subrel2)
            if rel_comp != 0:
                return rel_comp
    ...
    @classmethod
    def _group_packages_by_arch(cls, packages):
        """
    ...
    @classmethod
    def _compare_arch_packages(cls, self_packages, other_packages):
        """
        Compare packages.
        ...
        for name, pkg in self_packages.items():
            if name not in other_packages:
                ...
                if cls._compare_versions(ver1, ver2) < 0:
    ...
    def compare_packages(
        cls, self_branch_name, self_packages, other_branch_name, other_packages
    ):
        """
        ...
        """
        self_grouped = cls._group_packages_by_arch(self_packages)
        other_grouped = cls._group_packages_by_arch(other_packages)
        ...
        for arch in all_archs:
            self_arch_packages = {
                pkg["name"]: pkg for pkg in self_grouped.get(arch, [])
            }
            other_arch_packages = {
                pkg["name"]: pkg for pkg in other_grouped.get(arch, [])
            }

            self_only, other_only, version_diffs = cls._compare_arch_packages(
                self_arch_packages, other_arch_packages
            )
